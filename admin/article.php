...
if (isset($_POST['new'])) {
    ...
    if (isset($_FILES['image']) and $_FILES['image']['error'] != 4) {
        if ($_FILES['image']['error'] == 0) {
            ...
            if ( ($ext == "png" or $ext == "jpeg" or $ext == "bmp" or $ext == "jpg")) {
    ...
    if (isset($_POST['date'])) {
        ...
        if (count($ar) == 3 and checkdate($ar[0], $ar[1], $ar[2])) {
        ...
        if ($id_val == -1) {
            ...
            if (isset($_FILES['image']) and $_FILES['image']['error'] != 4) {
                ...
                SET t.Titre = ?,
                    ...
                    t.hidden = ?
                WHERE t.id = ?");
                $req->execute(array($titre_val, $texte_val, $date_val, $lieu_val,$hidden_val, $id_val));
            }
        }
...
<form enctype="multipart/form-data" method="post" action="/admin/article.php" class="container border border-secondary border-2 rounded-3 m-5 mx-auto p-2">
    ...
    <div class="form-check">
        <input class="form-check-input" type="checkbox" value="hide" id="hide" <?php if ($hidden_val) {echo "checked";}?>>
